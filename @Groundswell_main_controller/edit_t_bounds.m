function edit_t_bounds(gsmc)

% get the current tl_view
tl_view=gsmc.view.tl_view;  % always in s

% convert time limits in s to current time units
xl_view=gsmc.view.x_from_t(tl_view);

% get the string corresponding to current time units
str_units=gsmc.view.time_units_string();
