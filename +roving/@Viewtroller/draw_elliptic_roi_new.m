function draw_elliptic_roi_new(self,action)
...
switch(action)
  case 'start'
    ...
    set(ifig_h,'WindowButtonMotionFcn',...
               'roving.draw_elliptic_roi(''move'')');
    set(ifig_h,'WindowButtonUpFcn',...
               'roving.draw_elliptic_roi(''stop'')');
  case 'move'
    cp=get(image_axes_h,'CurrentPoint');
    point=cp(1,1:2)';
    ap=point-anchor;
  ...
  case 'stop'
    ...
    set(ifig_h,'WindowButtonMotionFcn','roving.update_pointer');
    set(ifig_h,'WindowButtonUpFcn','');
